...
class DoodleJump:
    def __init__(self):
        # Initialize Pygame
        pygame.init()
        self.screen_width = 1200  # Increased from 800
        self.screen_height = 800  # Increased from 600
        self.screen = pygame.display.set_mode((self.screen_width, self.screen_height))
        pygame.display.set_caption("Doodle Jump")
        
        # Set assets path relative to script location
        ...
        # Game variables
        self.score = 0
        self.playerx = self.screen_width // 2  # Start in center (600)
        self.playery = 500
    ...
    def load_assets(self):
        ...
        try:
            if not os.path.exists(self.assets_path):
                print(f"Error: Assets directory '{self.assets_path}' not found.")
                sys.exit(1)
                
            self.enemy_size = 70  # Enemy size (20x20 pixels)
            asset_files = {
                "green": "green.png",
                "blue": "blue.png",
                "red": "red.png",
            ...
            for attr, filename in asset_files.items():
                try:
                    ...
                    # Resize enemy image to 20x20 if it's the enemy asset
                    if attr == "enemy":
                        image = pygame.transform.scale(image, (self.enemy_size, self.enemy_size))
                    setattr(self, attr, image)
                except pygame.error as e:
                    print(f"Warning: Failed to load '{filepath}'. Using fallback image. Error: {e}")
                    if "player" in attr.lower():
                        setattr(self, attr, self.create_fallback_image(50, 50, (255, 0, 0)))  # Red for player
                    elif "spring" in attr.lower():
                        setattr(self, attr, self.create_fallback_image(30, 20, (128, 128, 128)))  # Gray for spring
                    elif "enemy" in attr.lower():
                        setattr(self, attr, self.create_fallback_image(self.enemy_size, self.enemy_size, (255, 0, 255)))  # Magenta for enemy
                    else:
                        setattr(self, attr, self.create_fallback_image(70, 10, (0, 255, 0)))  # Green for platforms
                        
        except Exception as e:
            print(f"Unexpected error loading assets: {e}")
            sys.exit(1)
    ...
    def updatePlayer(self):
        ...
        # Adjust camera to follow player (lower threshold for better visibility)
        if self.playery - self.cameray < 200:  # Changed from 300 to 200
            self.cameray = max(self.cameray - 10, self.playery - 200)  # Increased speed from 5 to 10
    ...
    def updateEnemies(self):
        ...
        for e in self.enemies[:]:
            ...
            if e[0] > self.screen_width - self.enemy_size:  # Updated for new width
                e[2] = 0
            elif e[0] < 0:
                e[2] = 1
    ...
    def generatePlatforms(self):
        ...
        y = self.screen_height - 200  # Adjusted for new height
        while y > -100:
            platform_type = random.choices([0, 1, 2], weights=[80, 15, 5], k=1)[0]
            x = random.randint(0, self.screen_width - 70)  # Updated for new width
            self.platforms.append([x, y, platform_type, 0])
